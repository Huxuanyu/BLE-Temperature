...
public class PeripheralActivity extends Activity implements BleWrapperUiCallbacks {	
    							    ...
    							    final List<BluetoothGattService> services)
    {
    	runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				mListType = ListType.GATT_SERVICES;
				mListView.setAdapter(mServicesListAdapter);
				mHeaderTitle.setText(mDeviceName + "的服务:");
				mHeaderBackButton.setVisibility(View.INVISIBLE);
    									   ...
    									   final List<BluetoothGattCharacteristic> chars)
    {
    	runOnUiThread(new Runnable() {
			...
			public void run() {
		    	...
		    	mHeaderTitle.setText(BleNamesResolver.resolveServiceName(service.getUuid().toString().toLowerCase(Locale.getDefault())) + "包含:");
										 ...
										 final BluetoothGattCharacteristic characteristic)
    {
    	runOnUiThread(new Runnable() {
			...
			public void run() {
		    	...
		    	mHeaderTitle.setText(BleNamesResolver.resolveCharacteristicName(characteristic.getUuid().toString().toLowerCase(Locale.getDefault())) + "的细节:");
		    	mHeaderBackButton.setVisibility(View.VISIBLE);
            					  ...
            					  final String description)
	{
		runOnUiThread(new Runnable() {
			@Override
			public void run() {
				Toast.makeText(getApplicationContext(), "写入 " + description + " 成功!", Toast.LENGTH_LONG).show();
			}
		});
	}
							  ...
							  final String description)
	{
		runOnUiThread(new Runnable() {
			@Override
			public void run() {
				Toast.makeText(getApplicationContext(), "写入 " + description + " 失败!", Toast.LENGTH_LONG).show();
			}
		});	
	}
